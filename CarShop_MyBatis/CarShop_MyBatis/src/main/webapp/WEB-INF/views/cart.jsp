			...
			<table class="table table-hover">
				...
				<form:form name="removeForm" method="put">
					<c:forEach items="${cart.cartItems}" var="item">
						<tr>
							<td><span class="cid">${item.value.car.cid}</span>-${item.value.car.cname}</td>
							<td>${item.value.car.cprice}</td>
							<td>${item.value.quantity}</td>
							<td>${item.value.totalPrice}</td>
<%-- 							<td><a href="javascript:removeFromCart('/cart/remove/${item.value.car.cid}')" class="btn btn-danger btn-sm">삭제</a></td> --%>
						<td><a href="javascript:removeCart('${item.value.car.cid}')" class="btn btn-danger btn-sm">삭제</a></td>
						</tr>
					</c:forEach>
				</form:form>
				<tr>
